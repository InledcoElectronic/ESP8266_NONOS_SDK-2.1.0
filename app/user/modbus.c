/*
 * modbus.c
 *
 *  Created on: 2017年9月11日
 *      Author: liruya
 */

#include "modbus.h"
#include "driver/uart0.h"

#define	MODBUS_BUFFER_SIZE	256

LOCAL const uint8_t auchCRCHi[] =
{  };
LOCAL const uint8_t auchCRCLo[] =
{  };
LOCAL uint8_t crcH = 0xFF;
LOCAL uint8_t crcL = 0xFF;
LOCAL uint32_t idx = 0;
LOCAL rx_buffer_t modbus_rx_buffer;

modbus_recv_frame_cb_t modbus_recv_frame_cb;

void ICACHE_FLASH_ATTR modbus_init( void )
{
	modbus_rx_buffer.buffer = (uint8_t *) os_zalloc( MODBUS_BUFFER_SIZE );
	uart0_init( BAUDRATE_9600, 32, &modbus_rx_buffer );
}

void ICACHE_FLASH_ATTR modbus_crc16_init( void )
{
	crcH = 0xFF;
	crcL = 0xFF;
	idx = 0;
}

uint16_t modbus_crc16_once( uint8_t byte )
{
	idx = crcL ^ byte;
	crcL = crcH ^ auchCRCHi[idx];
	crcH = auchCRCLo[idx];
	return ( crcH << 8 | crcL );
}

uint16_t ICACHE_FLASH_ATTR modbus_crc16( uint8_t *buf, uint32_t len )
{
	crcH = 0xFF;
	crcL = 0xFF;
	idx = 0;
	while ( len-- )
	{
		idx = crcL ^ *buf++;
		crcL = crcH ^ auchCRCHi[idx];
		crcH = auchCRCLo[idx];
	}
	return ( crcH << 8 | crcL );
}

uint16_t ICACHE_FLASH_ATTR modbus_checksum( void )
{
	return ( crcH << 8 | crcL );
}

void ICACHE_FLASH_ATTR modbus_send( uint8_t *pbuf, uint16_t len )
{
	uint16_t i;
	if( pbuf != NULL )
	{
		if( modbus_crc16( pbuf, len ) == 0x00 )
		{
			for ( i = 0; i < len; i++ )
			{
				uart0_send_byte( *( pbuf + i ) );
			}
		}
	}
}

void ICACHE_FLASH_ATTR modbus_receive( rx_buffer_t *pbuf )
{
	if ( pbuf != NULL )
	{
		if ( modbus_crc16( pbuf->buffer, pbuf->len ) == 0 )
		{
			if( modbus_recv_frame_cb )
			{
				modbus_recv_frame_cb( pbuf->buffer, pbuf->len );
			}
		}
	}
}

void ICACHE_FLASH_ATTR modbus_set_recv_frame_fn( modbus_recv_frame_cb_t callback )
{
	modbus_recv_frame_cb = callback;
}
